...
import { Post, CreatePostData, UpdatePostData } from '../../domain/entities/post.entity';
...
export class PostService implements IPostService {
  ...
  async create(data: CreatePostData): Promise<Post> {
    ...
    const existingSlugs = await this.postRepository.findSimilarSlugs(baseSlug);
  ...
  async findMany(query: PaginationQuery & SearchQuery): Promise<PaginationResult<Post>> {
    return await this.postRepository.findMany(query);
  }

  async findByAuthor(authorId: string, query: PaginationQuery): Promise<PaginationResult<Post>> {
  ...
  async update(id: string, data: UpdatePostData): Promise<Post> {
    ...
    if (data.title && data.title !== existingPost.title) {
      const baseSlug = SlugUtil.generate(data.title);
      const existingSlugs = await this.postRepository.findSimilarSlugs(baseSlug);
      updateData.slug = SlugUtil.generateUnique(data.title, existingSlugs.filter(s => s !== existingPost.slug));
    }
  ...
  async search(searchTerm: string, query: PaginationQuery): Promise<PaginationResult<Post>> {
